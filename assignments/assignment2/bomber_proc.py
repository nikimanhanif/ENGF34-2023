...
def shrink_building(canvas, building_num, building_width, building_heights, building_xpos, building_rects):
    #ensure building height never becomes negative
    if building_heights[building_num] - 50 > 0:
...
def display_score(canvas, score, level):
    ...
    canvas.itemconfig(score_text, text="Level: " + str(level) + "  Score: " + str(score), font=scorefont, fill="blue")
...
def create_buildings(canvas, building_width, building_heights, building_xpos, building_rects):
    ...
    if len(building_rects) > 0:
        for building_num in range(0, 900//SPACING): #adjusted the range
            delete_building(canvas, building_num, building_rects)
    building_heights.clear()
    building_xpos.clear()

    #create the new ones
    for building_num in range(0, (900//SPACING)): #adjusted the range to not include the last building
...
def check_bomb(canvas, bomb_pos, building_width, building_heights, building_xpos, building_rects):
    ...
    for building_num in range(0, len(building_xpos)): #adjusted to follow the range
        ...
        elif bomb_pos[1] >= CANVAS_HEIGHT: #added a conditional statement where if the y position of the bomb exceeded the canvas, the bomb would reset
            explode()
...
def check_plane(canvas, plane_pos, building_width, building_heights, building_xpos):
    ...
    for building_num in range(0, len(building_xpos)): #adjusted to follow range
        if (is_inside_building(building_num, plane_nose_pos, building_width,
            ...
            or is_inside_building(building_num, plane_wing_pos, building_width,
                                  building_heights, building_xpos)) :
            #adjusted for the new conditional statement
...
def game_over(canvas):
    ...
    canvas.itemconfig(msg_text, text="GAME OVER!", font=bigfont, fill="blue") #i've changed the game over message to blue to make it easier to see
